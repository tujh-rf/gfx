/*
    ...
    Direct3D9 Simple Fullscreen application
 */
...
static bool init_window( d3dApp &app ) {
    ...
    app.glfw.window = glfwCreateWindow(
        window_width,
        window_height,
        "Direct3D9 - Tutorial - fullscreen", /* window title */
        nullptr,                             /* nothing here,
                                                Direct3D9 will take case about fullscreen mode */
        nullptr
    );
    if( !app.glfw.window )
        return false;
...
static bool init_d3d9( d3dApp &app ) {
    ...
    UINT mode_count = app.d3d9.instance->GetAdapterModeCount( D3DADAPTER_DEFAULT,
                                                              D3DFMT_X8R8G8B8 );

    if( !mode_count )
        return false;

    bool is_mode_found = false;
    for( UINT i = 0; i < mode_count; ++i ) {
        D3DDISPLAYMODE d3ddm;
        if( FAILED( app.d3d9.instance->EnumAdapterModes( D3DADAPTER_DEFAULT,
                                                         D3DFMT_X8R8G8B8,
                                                         i,
                                                        &d3ddm ) ) )
            return false;

        if( d3ddm.Width != window_width )   /* full screen resolution - width */
            continue;
        if( d3ddm.Height != window_height ) /* full screen resolution - height */
            continue;
        if( d3ddm.Format != D3DFMT_X8R8G8B8 ) /* full screen pixel format */
            continue;
        if( d3ddm.RefreshRate != 60 )       /* full screen refresh rate, 60Hz usually supported everywhere */
            continue;
    ...
    /* test fullscreen buffer formats
     */
    if( FAILED( app.d3d9.instance->CheckDeviceType( D3DADAPTER_DEFAULT,
                                                    D3DDEVTYPE_HAL,
                                                    D3DFMT_X8R8G8B8,
                                                    D3DFMT_X8R8G8B8,
                                                    FALSE ) ) )
    ...
    /* test XRGB pixel format with the 16 Stencil format support
     */
    ...
    app.d3d9.d3dpp.hDeviceWindow          = glfwGetWin32Window( app.glfw.window );
    app.d3d9.d3dpp.SwapEffect             = D3DSWAPEFFECT_DISCARD; /* no multisampling for now */
    app.d3d9.d3dpp.BackBufferWidth        = static_cast< UINT > ( window_width );
    app.d3d9.d3dpp.BackBufferHeight       = static_cast< UINT > ( window_height );
    app.d3d9.d3dpp.BackBufferFormat       = D3DFMT_X8R8G8B8;
    app.d3d9.d3dpp.Windowed               = FALSE;
    ...
    app.d3d9.d3dpp.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;
