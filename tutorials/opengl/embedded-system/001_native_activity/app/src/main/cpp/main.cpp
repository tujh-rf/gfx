...
static void android_looper_poll( struct android_app *native_ptr ) {
    int events;
    struct android_poll_source *source_ptr = nullptr;
...
void android_main( struct android_app *native_ptr ) {
    ...
    do {
        ...
        if( app && app->activity_init ) {
            ...
            destroy_requested = !!native_ptr->destroyRequested;
        }
